...
- [ ] 3.1 Create `src/mango/merger.py` with a `merge_configs(config_dir: Path) -> list[str]` function that applies category merge rules (exact match → merge macros, new category → include entirely, partial match → omit + collect warning)

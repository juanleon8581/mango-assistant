...
TBD — Merge `config.default.yaml` and `config.local.yaml` into `commands.yaml` at startup using lazy hash-based change detection, with well-defined conflict rules for categories and macros.
